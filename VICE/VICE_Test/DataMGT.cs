...
namespace VICE_Final
{
    public partial class DataMGT : Form
    {
        ...
        float voltageForPower;
        float currentForPower;
        ...
        public void parseVal(string letter, string value)
        {
            ...
            string voltagetop = "SP14VICE_TopVoltage"; //"F13APA_Voltage";
            string currenttop = "SP14VICE_TopCurrent"; //"F13APA_CurrentTOP";
            string currentbot = "F13APA_CurrentBOTTOM";
            string powertop = "SP14VICE_TopPower"; //"F13APA_Power_Top";
            ...
            switch (letter)                        //checks first value in string for command
            {
                ...
                case "DF":                          //if DesiredFlow
                    ...
                    break;
                case "vt":                          //if voltage
                    string vtemp = voltagetop + outlet.ToString();
                    try
                    {
                        myPI.connect_Server("ESMARTSERVER-PC");
                        ...
                        {
                            //Setting the value of the point
                            myPI.setPiPointValue(vtemp, (((float.Parse(value) / (Math.Pow(2, 24) - 1)) * 120) / 0.6));
                            voltageForPower = (float)(((float.Parse(value) / (Math.Pow(2, 24) - 1)) * 120) / 0.6);
                        }
                    }
                    catch (Exception ex)
                    {
                ...
                case "it":                            //if current TOP outlet
                    ...
                    try
                    {
                        ...
                        if (myPI.check_connection())
                        {
                            ...
                            currentForPower = (float)((((float.Parse(value))*15)/(Math.Pow(2,24)-1))/.6);
                        }
                ...
                case "pt":                            //POWER TOP
                    ...
                    try
                    {
                        ...
                        {
                            //Setting the value of the point
                            myPI.setPiPointValue(powertoptemp, voltageForPower * currentForPower);
                        }
                    }
                    catch (Exception ex)
                    {
